...
bool esValida(int k, int c, std::vector <int>& sol, const std::vector <int>& apar) {
    if (c == 2 && sol[k - 1] == 2) 
        return false;
    if (apar[2] > apar[0])
        return false;
...
bool esSolvalida(std::vector <int>& sol, const std::vector <int>& apar, const int& a, const int& r, const int& v) {
...
int resolver(const int& m, const int& n, std::vector <int>& sol, const int& k, std::vector <int>& apar, const int& a, const int& r, const int& v) {
    ...
    for (int c = 0; c < n; c++) {
        sol[k] = c;
        apar[c]++;
        if (esValida(k, c, sol,apar)) {
            if (k == sol.size() - 1) {
                if (esSolvalida(sol,apar,a,r,v)) {
                    escribirsol(sol);
                    numsol++;
                }
            }
